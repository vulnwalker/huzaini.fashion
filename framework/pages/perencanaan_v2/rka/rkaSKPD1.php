	...
	function set_selector_other($tipe){
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){	
		...
		case 'clearTemp':{
				$username =$_COOKIE['coID'];
				mysql_query("delete from temp_rka_1_v2 where user ='$username'");	
				foreach ($_REQUEST as $key => $value) { 
			 	 $$key = $value; 
				}
		...
		case 'editTab':{
			...
			while($rows = mysql_fetch_array($getAll)){
				...
				$getMaxID = mysql_fetch_array(mysql_query("select  max(id) as gblk from temp_rka_1_v2 where user ='$username'"));
				$maxID = $getMaxID['gblk'];
				$lastO1 = $o1;
				
				$getLastO1 = mysql_fetch_array(mysql_query("select o1 from temp_rka_1_v2 where id='$maxID' "));
