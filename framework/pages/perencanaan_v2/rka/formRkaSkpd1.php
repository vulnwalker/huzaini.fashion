	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){	
		...
		case 'finish':{		
			...
			}elseif(mysql_num_rows(mysql_query("select * from temp_rka_1_v2  where user ='$username' and delete_status = '0'")) == 0){
				$err = "Data kosong";
			}
			
			if(empty($err)){
				$execute = mysql_query("select * from temp_rka_1_v2  where user ='$username' ");
		...
		case 'SaveJob':{
			...
			$getMaxUrut = mysql_fetch_array(mysql_query("select max(urut) from temp_rka_1_v2 where user ='$username'"));
		...
		case 'SaveEditJob':{
			...
			$getUrut = mysql_fetch_array(mysql_query("select * from temp_rka_1_v2 where o1='$o1'"));
			$urut = $getUrut['urut'];
		...
		case 'cekNoUrut':{
	    	foreach ($_REQUEST as $key => $value) { 
			 ...
			 $username = $_COOKIE['coID'];
			 $cekRow = mysql_num_rows(mysql_query("select * from temp_rka_1_v2  where o1 = '$noPekerjaan'   and user ='$username' "));
			 if($cekRow == 0){
			 	$get = mysql_fetch_array(mysql_query("select max(urut) from temp_rka_1_v2  where  user ='$username' and delete_status !='1' and o1 !='0' "));
			 	$urut = $get['max(urut)'] + 1;
			 }else{
			 	 $get = mysql_fetch_array(mysql_query("select * from temp_rka_1_v2 where o1 = '$noPekerjaan'   and user ='$username' "));
				 $urut = $get['urut'];
			 }
		...
		case 'hapus2':{
	    	 $id = $_REQUEST['id'];
			 $get = mysql_fetch_array(mysql_query("select * from temp_rka_1_v2 where id='$id' "));
			 $username = $_COOKIE['coID'];
			 $noPekerjaan = $get['o1'];
			 $noUrutPekerjaan = $get['urut'];
			 mysql_query("update  temp_rka_1_v2 set delete_status = '1', o1 ='0' where id='$id'");
			 $execute = mysql_query("select * from temp_rka_1_v2  where user='$username' and o1='$noPekerjaan' and delete_status = '0' order by o1, rincian_perhitungan");
			 $angkaUrut = 1;
			 while($rows = mysql_fetch_array($execute)){
				...
				$dataEditNoUrut = array('urut' => $noUrutPekerjaan,
			 						 	'o2' => $angkaUrut);
				$idTemp = $rows['id'];
				mysql_query(VulnWalkerUpdate("temp_rka_1_v2",$dataEditNoUrut," id='$idTemp'"));
   ...
   function setPage_OtherScript(){
	...
	$Koloms[] = array('align="center" width="10"',$no );
		if($f != '00' || $rincian_perhitungan != ''){
			...
			$getTotal = mysql_fetch_array(mysql_query("select sum(volume_rek) as volRek, sum(jumlah_harga) as jumlahHarga from temp_rka_1_v2 where o1 ='$o1' and user='$username'  and delete_status !='1'"));
	...
	function getDaftarOpsi($Mode=1){
		...
		$arrKondisi = array();		
		$username = $_COOKIE['coID'];
		$getAll = mysql_query("select * from temp_rka_1_v2 where rincian_perhitungan ='' and user = '$username' and o1!='0'");
		while($rows = mysql_fetch_array($getAll)){
			foreach ($rows as $key => $value) { 
				 	 	$$key = $value; 
			}
			if(mysql_num_rows(mysql_query("select * from temp_rka_1_v2 where user ='$username' and rincian_perhitungan !='' and o1='$o1' and delete_status = '0' ")) == 0){
				mysql_query("delete from temp_rka_1_v2 where id='$id'");				
			}else{
