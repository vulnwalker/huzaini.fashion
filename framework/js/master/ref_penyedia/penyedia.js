var penyedia = new DaftarObj2({
	prefix : 'penyedia',
	url : 'pages.php?Pg=ref_penyedia', 
	formName : 'penyediaForm',
	penyedia_form : '0',
	loading: function(){
		//alert('loading');
		this.topBarRender();
	...
	Baru: function(){	
		...
		if (err =='' ){		
			...
			if(me.penyedia_form==0){//baru dari penyedia
		...
		$.ajax({
			...
			url: this.url+'&tipe=simpan',
		  	success: function(data) {		
				...
				//document.getElementById(cover).innerHTML = resp.content;
				if(resp.err==''){
					if(me.penyedia_form==0){
						me.Close();
						me.AfterSimpan();						
					}else{
						me.Close();
						barang.refreshCombopenyedia();
					}
