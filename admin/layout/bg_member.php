...
<?php
$sql = query("SELECT id_user,nama_lengkap,email,tlp,alamat,dayofmonth(bergabung_tgl) AS tgl, month(bergabung_tgl) AS bln ,year(bergabung_tgl) as thn from user WHERE level ='member' order by id_user ASC");
  $no = 1;
  while ($data=fetch($sql)) {
    ...
    echo'
        <tr>
          ...
          location.href=\'proses.php?act=hapus_member&id='.$data['id_user'].'\';">    
