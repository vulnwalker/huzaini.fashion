<?php
    $a=mysql_query("select max(id_kategori) as id_max from kategori");
...
<?php
  $kode=$_GET['id'];
  $query = mysql_query("SELECT *FROM kategori WHERE id_kategori='$kode'");
            ...
            <div class="judul-set"><i class="fa fa-product-hunt"></i> Edit Kategori</div>
             <?php echo"<form method='post' action='proses.php?act=edit_kategori'>"?>            
                <div class="wadah">
                    <div>
                      <label>Id Kategori</label>
                      <?php  echo"<input type='text' class='input not' name='id_kategori' value='$data[0]' readonly />"?> 
                    </div>             
                    <div>
                      <label>Nama</label>
                      <?php echo'<input type="text" class="input" value="'.$data[1].'"  name="nama" placeholder="Nama" required />';?>
                    </div>            
                    <div>
                      <label style="float:left; width:100px;">Keterangan</label>
                      <?php echo'<textarea class="input ket" placeholder="Keterangan" name="keterangan" required>'.$data[2].'</textarea>'?>
                    </div>                          
                    <div>
                       <button type="submit" class="btn-save"><i class="fa fa-save"></i> Simpan</button>
                       <button type="reset"  class="btn-reset"><i class="fa fa-times"></i> Batal</button>
