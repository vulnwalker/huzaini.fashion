<?php
    $a=mysql_query("select max(id_merek) as id_max from merek");
...
<?php
  $kode=$_GET['id'];
  $query = mysql_query("SELECT *FROM merek WHERE id_merek='$kode'");
            ...
            <div class="judul-set"><i class="fa fa-product-hunt"></i> Edit Merek</div>
             <?php echo"<form method='post' enctype='multipart/form-data' action='proses.php?act=edit_merek'>"?>            
                <div class="wadah">
                    <div>
                      <label>Id Merek</label>
                      <?php  echo"<input type='text' class='input not' name='id_merek' value='$data[0]' readonly />"?> 
                    </div>             
                    <div>
                      <label>Nama Merek</label>
