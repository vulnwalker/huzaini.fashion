...
switch ($hal) {  
  default:
    $sql = mysql_query("SELECT *FROM user,order_produk where user.id_user = order_produk.id_pembeli AND order_produk.konfirmasi_order='sudah' order by order_produk.tgl_order DESC");
  ...
  case 'selected':
    $sql = mysql_query("SELECT *FROM user,order_produk where user.id_user = order_produk.id_pembeli AND order_produk.konfirmasi_order='sudah' AND order_produk.tgl_order >= DATE_SUB(curdate(),INTERVAL $banyak day) order by order_produk.tgl_order DESC");
    while ($data=mysql_fetch_array($sql)) {
    echo'
        <tr>
          <td>'.$data[id_order].'</td>
