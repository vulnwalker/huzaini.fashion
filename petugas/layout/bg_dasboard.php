...
$sqlusr = mysql_query("SELECT COUNT(*) as usr from user Where level='member'");
$cekusr = mysql_fetch_assoc($sqlusr);


$sqlpk = mysql_query("SELECT COUNT(*) as pk from produk");
$cekpk = mysql_fetch_assoc($sqlpk);


$sqlop = mysql_query("SELECT COUNT(*) as op from order_produk");
$cekop = mysql_fetch_assoc($sqlop);
          ...
          <div id="titledas1">Member Baru</div>
              <table id="t01" class="beda02">
                ...
                <?php
                  ...
                  $hasil =mysql_query($sqlm);
                    $no = 1;
                    while ($datam=mysql_fetch_array($hasil)) {
                       $bln= $datam[bln];
                      echo'
                          <tr>
                            <td>'.$datam[id_user].'</td>
